...
export function recommendedWorkerCount(
  ...
  return normalizeWorkerCount(
    Math.min(available, DEFAULT_AUTO_WORKER_CAP),
    DEFAULT_MAX_WORKERS,
  );
}
...
export class AnalysisWorkerPool {
  ...
  async resize(value) {
    ...
    try {
      while (this.#workers.length < targetSize) {
        ...
        this.#workers.push(new WorkerClient(
          this.#workerFactory,
          this.#workerUrl,
          `anarcism-${index + 1}`,
        ));
  ...
  async #numberInputs(inputs, options) {
    ...
    try {
      ...
      const settledResponses = await Promise.allSettled(shards.map((shard, index) =>
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    if (line.startsWith(">")) {
      ...
      if (!id) {
        throw makeError(
          "INVALID_FASTA",
          `FASTA header on line ${lineIndex + 1} is empty`,
        );
      }
      currentId = id;
      currentSequence = [];
