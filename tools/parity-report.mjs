...
const expandedReferenceUrl = new URL(
...
function numberingFingerprint(numbering) {
  return fnv1a64(numbering.map((residue) => [
    residue.sequenceIndex,
    residue.aminoAcid,
    residue.position,
...
async function checkExpandedShard(shardIndex, shardCount) {
  ...
  for (let caseIndex = shardIndex; caseIndex < expandedCorpus.length; caseIndex += shardCount) {
    ...
    for (let index = 0; index < observed.domains.length; index += 1) {
      ...
      const numberingExact = actual.numbering.length === expected.numberingLength
...
if (!isMainThread) {
  ...
  const expandedPromise = expandedCorpus.length === expandedReference.cases.length
    ? Promise.all(Array.from(
      { length: workerCount },
      (_, shardIndex) => runWorker(shardIndex, workerCount),
    ))
    : Promise.resolve([]);
